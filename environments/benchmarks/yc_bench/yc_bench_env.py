...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs OPENMORKAgentLoop with terminal tool only
...
class YCBenchEvalConfig(OPENMORKAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends OPENMORKAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(OPENMORKAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The OPENMORKAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs OPENMORKAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our OPENMORKAgentLoop.
            ...
            # ----------------------------------------------------------
            # Step 2: Run the OPENMORKAgentLoop
            # ----------------------------------------------------------
