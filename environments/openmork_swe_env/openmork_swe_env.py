"""
OPENMORKSweEnv -- SWE-Bench Style Environment with Modal Sandboxes
...
Usage:
    ...
    vllm serve YourModel --tool-parser openmork
    run-api
    python environments/openmork_swe_env.py serve \\
...
class OPENMORKSweEnv(OPENMORKAgentBaseEnv):
    ...
    name = "openmork-swe"
    env_config_cls = OPENMORKSweEnvConfig

    @classmethod
    def config_init(cls) -> Tuple[OPENMORKSweEnvConfig, List[APIServerConfig]]:
        """
        ...
        """
        env_config = OPENMORKSweEnvConfig(
            ...
            # Atropos settings
            group_size=4,
            tokenizer_name="NousResearch/DeepOPENMORK-3-Llama-3-3B-Preview",
            tool_call_parser="openmork",
            steps_per_eval=50,
            total_steps=500,
            use_wandb=True,
            wandb_name="openmork-swe",
        )

        server_configs = [
            APIServerConfig(
                base_url="http://localhost:8000/v1",
                model_name="NousResearch/DeepOPENMORK-3-Llama-3-3B-Preview",
