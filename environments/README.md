...
This directory contains the integration layer between **openmork's** tool-calling capabilities and the **Atropos** RL training framework. It provides everything needed to run agentic LLMs through multi-turn tool-calling loops, score their output with arbitrary reward functions, and feed results into Atropos for training or evaluation.
...
```
                    ...
                    └───────────┬───────────┘
                                │ inherits
                    ┌───────────┴───────────┐
                    │  OPENMORKAgentBaseEnv    │  openmork_base_env.py
                    │  - Terminal backend    │
                    │  - Tool resolution     │
                    │  - Agent loop          │
                    │  - ToolContext          │
              ...
              ┌─────────────────┼─────────────────┐
              │                 │                  │
     TerminalTestEnv     OPENMORKSweEnv    TerminalBench2EvalEnv
...
**OPENMORKAgentBaseEnv** (`openmork_base_env.py`) extends BaseEnv with openmork specifics:
- Sets `os.environ["TERMINAL_ENV"]` to configure the terminal backend (local, docker, modal, daytona, ssh, singularity)
- Resolves openmork toolsets via `_resolve_tools_for_group()` (calls `get_tool_definitions()` which queries `tools/registry.py`)
- Implements `collect_trajectory()` which runs the full agent loop and computes rewards
- Supports two-phase operation (Phase 1: OpenAI server, Phase 2: VLLM ManagedServer)
- Applies monkey patches for async-safe tool operation at import time

Concrete environments inherit from `OPENMORKAgentBaseEnv` and implement:
...
`OPENMORKAgentLoop` is the reusable multi-turn agent engine. It runs the same pattern as openmork's `run_agent.py`:
...
`ToolContext` is a per-rollout handle that gives reward/verification functions direct access to **all** openmork tools, scoped to the rollout's `task_id`. The same `task_id` means the terminal/browser session is the SAME one the model used during its rollout -- all state (files, processes, browser tabs) is preserved.
...
- **Generic**: `call_tool(name, args)` -- call any openmork tool by name
- **Cleanup**: `cleanup()` -- release all resources (called automatically after `compute_reward`)

### Patches (`patches.py`)

**Problem**: Some openmork tools use `asyncio.run()` internally (e.g., mini-swe-agent's Modal backend via SWE-ReX). This crashes when called from inside Atropos's event loop because `asyncio.run()` cannot be nested.
...
Applied automatically at import time by `openmork_base_env.py`.
...
Available parsers:
- `openmork` -- OPENMORK/ChatML `<tool_call>` XML format
...
├── openmork_base_env.py            # Abstract base (OPENMORKAgentBaseEnv)
├── agent_loop.py                 # Multi-turn agent engine (OPENMORKAgentLoop)
...
│
├── openmork_swe_env/               # SWE-bench style training environment
│   └── openmork_swe_env.py
│
...
### OPENMORKSweEnv (`openmork_swe_env/`)

SWE-bench style training environment. The model gets a coding task, uses terminal + file + web tools to solve it, and the reward function runs tests in the same Modal sandbox.

```bash
python environments/openmork_swe_env/openmork_swe_env.py serve \
...
2. Create your env file inheriting from `OPENMORKAgentBaseEnv`
...
class MyEnvConfig(OPENMORKAgentEnvConfig):
    pass  # Add custom fields as needed

class MyEnv(OPENMORKAgentBaseEnv):
    name = "my-env"
...
2. Inherit from `OPENMORKAgentBaseEnv`
...
|-------|-------------|---------|
| `enabled_toolsets` | Which openmork toolsets to enable | `None` (all) |
...
| `tool_call_parser` | Parser name for Phase 2 | `openmork` |
