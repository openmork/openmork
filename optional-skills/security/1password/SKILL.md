...
version: 1.0.0
author: arceus77-7, enhanced by OpenMork
license: MIT
metadata:
  openmork:
...
- `tmux` available for stable authenticated sessions during OPENMORK terminal calls (desktop app flow only)
...
### Service Account (recommended for OPENMORK)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.openmork/.env` (the skill will prompt for this on first load).
...
## OPENMORK Execution Pattern (desktop app flow)

OPENMORK terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/openmork-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/openmork-op.sock"
