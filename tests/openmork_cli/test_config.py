"""Tests for openmork_cli configuration management."""
...
class TestGetOPENMORKHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("OPENMORK_HOME", None)
            home = get_openmork_home()
            assert home == Path.home() / ".openmork"

    def test_env_override(self):
        with patch.dict(os.environ, {"OPENMORK_HOME": "/custom/path"}):
            home = get_openmork_home()
            assert home == Path("/custom/path")


class TestEnsureOPENMORKHome:
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"OPENMORK_HOME": str(tmp_path)}, clear=False):
