"""Tests for the update check mechanism in openmork_cli.banner."""
...
def test_version_string_no_v_prefix():
    ...
    from openmork_cli import __version__
...
def test_check_for_updates_uses_cache(tmp_path):
    ...
    with patch("openmork_cli.banner.os.getenv", return_value=str(tmp_path)):
        with patch("openmork_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path):
    ...
    with patch("openmork_cli.banner.os.getenv", return_value=str(tmp_path)):
        with patch("openmork_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
            result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path):
    ...
    import openmork_cli.banner as banner

    # Create a fake banner.py so the fallback path also has no .git
    fake_banner = tmp_path / "openmork_cli" / "banner.py"
...
def test_check_for_updates_fallback_to_project_root():
    """Dev install: falls back to Path(__file__).parent.parent when OPENMORK_HOME has no git repo."""
    import openmork_cli.banner as banner
