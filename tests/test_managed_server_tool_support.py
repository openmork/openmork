...
Validates that:
1. The installed atroposlib API still matches OPENMORK's expectations
...
class TestBaseEnvCompatibility:
    """Test that openmork_base_env.py's tool-parser wiring matches the current API."""

    def test_openmork_base_env_sets_server_manager_tool_parser(self):
        """OPENMORK wires parser selection through ServerManager.tool_parser."""
        import ast

        base_env_path = Path(__file__).parent.parent / "environments" / "openmork_base_env.py"
        ...
        assert found_assignment, (
            "openmork_base_env.py should set self.server.tool_parser from config.tool_call_parser"
        )

    def test_openmork_base_env_uses_config_tool_call_parser(self):
        """Verify openmork_base_env uses the config field rather than a local parser instance."""
        base_env_path = Path(__file__).parent.parent / "environments" / "openmork_base_env.py"
        source = base_env_path.read_text()
