...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._openmork_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_openmork_bin(self):
        """After _INSTALL_FAILED, manual install in OPENMORK_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        openmork_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(openmork_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(openmork_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._openmork_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == openmork_bin
            assert _tirith_mod._resolved_path == openmork_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# OPENMORK_HOME isolation
# ---------------------------------------------------------------------------

class TestOPENMORKHomeIsolation:
    def test_openmork_bin_dir_respects_openmork_home(self):
        """_openmork_bin_dir must use OPENMORK_HOME, not hardcoded ~/.openmork."""
        from tools.tirith_security import _openmork_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"OPENMORK_HOME": tmpdir}):
            result = _openmork_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_openmork_home(self):
        """_failure_marker_path must use OPENMORK_HOME, not hardcoded ~/.openmork."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"OPENMORK_HOME": "/custom/openmork"}):
            result = _failure_marker_path()
        assert result == "/custom/openmork/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets OPENMORK_HOME; verify it's active."""
        openmork_home = os.getenv("OPENMORK_HOME")
        assert openmork_home is not None, "OPENMORK_HOME should be set by conftest"
        assert "openmork_test" in openmork_home, "Should point to test temp dir"

    def test_get_openmork_home_fallback(self):
        """Without OPENMORK_HOME set, falls back to ~/.openmork."""
        from tools.tirith_security import _get_openmork_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove OPENMORK_HOME entirely
            os.environ.pop("OPENMORK_HOME", None)
            result = _get_openmork_home()
        assert result == os.path.join(os.path.expanduser("~"), ".openmork")
