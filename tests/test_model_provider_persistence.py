"""Tests that provider selection via `openmork model` always persists correctly.
...
def config_home(tmp_path, monkeypatch):
    """Isolated OPENMORK_HOME with a minimal string-format config."""
    ...
    monkeypatch.delenv("OPENMORK_MODEL", raising=False)
    monkeypatch.delenv("LLM_MODEL", raising=False)
    monkeypatch.delenv("OPENMORK_INFERENCE_PROVIDER", raising=False)
...
class TestSaveModelChoiceAlwaysDict:
    def test_string_model_becomes_dict(self, config_home):
        ...
        from openmork_cli.auth import _save_model_choice
...
class TestProviderPersistsAfterModelSave:
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        from openmork_cli.main import _model_flow_api_key_provider
        ...
        with patch("openmork_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("openmork_cli.auth.deactivate_provider"), \
