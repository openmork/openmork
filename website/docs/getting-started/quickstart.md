...
openmork setup       # Or configure everything at once
```

`openmork model` walks you through selecting an inference provider:

| Provider | What it is | How to set up |
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `openmork model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `openmork model` |
| **Anthropic** | Claude models directly (Pro/Max or API key) | `openmork model` with Claude Code auth, or an Anthropic API key |
...
:::tip
You can switch providers at any time with `openmork model` — no code changes, no lock-in.
:::
...
When you exit, openmork prints a resume command:

```bash
openmork --continue    # Resume the most recent session
...
```bash
openmork config set terminal.backend docker    # Docker isolation
openmork config set terminal.backend ssh       # Remote server
```

### Connect messaging platforms

Chat with OPENMORK from your phone or other surfaces via Telegram, Discord, Slack, WhatsApp, Signal, Email, or Home Assistant:

```bash
openmork gateway setup    # Interactive platform configuration
```
...
Then start OPENMORK and enable it inside the CLI:

```text
/voice on
```

Press `Ctrl+B` to record, or use `/voice tts` to have OPENMORK speak its replies. See [Voice Mode](../user-guide/features/voice-mode.md) for the full setup across CLI, Telegram, Discord, and Discord voice channels.
...
```bash
openmork skills search kubernetes
...
### Use OPENMORK inside an editor via ACP

OPENMORK can also run as an ACP server for ACP-compatible editors like VS Code, Zed, and JetBrains:
...
| `openmork model` | Choose your LLM provider and model |
| `openmork tools` | Configure which tools are enabled per platform |
| `openmork setup` | Full setup wizard (configures everything at once) |
| `openmork doctor` | Diagnose issues |
| `openmork update` | Update to latest version |
| `openmork gateway` | Start the messaging gateway |
| `openmork --continue` | Resume last session |
