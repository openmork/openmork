...
The ACP adapter wraps OPENMORK' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
openmork acp / openmork-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> load ~/.openmork/.env
  -> configure stderr logging
  -> construct OPENMORKACPAgent
  -> acp.run_agent(agent)
```
...
- `allow_once` -> OPENMORK `once`
- `allow_always` -> OPENMORK `always`
- reject options -> OPENMORK `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps OPENMORK tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["openmork-acp"])
...
Instead it reuses OPENMORK' runtime resolver:

- `acp_adapter/auth.py`
- `openmork_cli/runtime_provider.py`

So ACP advertises and uses the currently configured OPENMORK provider/credentials.
...
- `toolsets.py` — `openmork-acp` toolset definition
- `openmork_cli/main.py` — `openmork acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `openmork-acp` script
