...
2. **`toolsets.py`** — add tool name to `_OPENMORK_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `openmork_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `openmork_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `openmork chat -q "Use the weather tool for London"`
