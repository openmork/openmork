...
├── openmork_state.py           # SQLite session/state database
├── batch_runner.py           # batch trajectory generation
│
├── agent/                    # prompt building, compression, caching, metadata, trajectories
├── openmork_cli/               # command entrypoints, auth, setup, models, config, doctor
...
├── environments/             # OPENMORK RL / benchmark environment framework
...
OPENMORK has a shared runtime provider resolver used by CLI, gateway, cron, ACP, and auxiliary calls.
...
ACP exposes OPENMORK as an editor-native agent over stdio/JSON-RPC.
...
OPENMORK ships a full environment framework for evaluation, RL integration, and SFT data generation.
...
The older mental model of OPENMORK as “one OpenAI-compatible chat loop plus some tools” is no longer sufficient. Current OPENMORK includes:
