...
`SOUL.md` is the easiest way to give OPENMORK a stable, default voice.

If you want OPENMORK to feel like the same assistant every time you talk to it — without repeating instructions in every session — this is the file to use.
...
- personality
- communication style
- how direct or warm OPENMORK should be
- what OPENMORK should avoid stylistically
- how OPENMORK should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who OPENMORK is and how OPENMORK speaks
...
OPENMORK now uses only the global SOUL file for the current instance:

```text
~/.openmork/SOUL.md
```

If you run OPENMORK with a custom home directory, it becomes:
...
OPENMORK automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, OPENMORK does not overwrite it
- if the file exists but is empty, OPENMORK adds nothing from it to the prompt

## How OPENMORK uses it

When OPENMORK starts a session, it reads `SOUL.md` from `OPENMORK_HOME`, scans it for prompt-injection patterns, truncates it if needed, and injects the content directly into the prompt.

No wrapper language is added around the file.

So the content itself matters. Write the way you want OPENMORK to think and speak.
...
That alone can noticeably change how OPENMORK feels.
...
OPENMORK already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How OPENMORK should sound.

# Avoid
What OPENMORK should not do.

# Defaults
How OPENMORK should behave when ambiguity appears.
```
...
```bash
nano ~/.openmork/SOUL.md
```

or

```bash
vim ~/.openmork/SOUL.md
```

Then restart OPENMORK or start a new session.
...
4. Talk to OPENMORK for a while
...
### I edited SOUL.md but OPENMORK still sounds the same

Check:
- you edited `~/.openmork/SOUL.md` or `$OPENMORK_HOME/SOUL.md`
...
### OPENMORK is ignoring parts of my SOUL.md
