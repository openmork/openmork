...
2. **OPENMORK spins up** a fresh agent session with your prompt
...
- **OpenMork installed** — see the [Installation guide](/docs/getting-started/installation)
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.openmork/cron/output/` and you can read them anytime.
:::
...
OPENMORK will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by OpenMork
```
...
Just tell OPENMORK what you want:
...
OPENMORK will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell OPENMORK to delegate each topic to a sub-agent:
...
OPENMORK will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
