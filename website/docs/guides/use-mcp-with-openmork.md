...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native OPENMORK tool
- you want OPENMORK to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect OPENMORK to internal APIs, databases, or company systems without modifying OPENMORK core

Do not use MCP when:
- a built-in OPENMORK tool already solves the job well
...
- OPENMORK remains the agent
- MCP servers contribute tools
- OPENMORK discovers those tools at startup or reload time
...
- OPENMORK banner/status should show MCP integration when configured
- ask OPENMORK what tools it has available
...
There are two categories of MCP-exposed functionality in OPENMORK:
...
2. OPENMORK-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So OPENMORK will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want OPENMORK to reason over a bounded workspace.
...
Now OPENMORK can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing OPENMORK core.
...
Because OPENMORK now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
