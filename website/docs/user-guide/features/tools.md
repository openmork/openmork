...
OPENMORK ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, RL training, messaging delivery, Home Assistant, Honcho memory, and more.
...
openmork chat --toolsets "web,terminal"
...
See [Toolsets Reference](/docs/reference/toolsets-reference) for the full set, including platform presets such as `openmork-cli`, `openmork-telegram`, and dynamic MCP toolsets like `mcp-<server>`.
...
```
```bash
# Set credentials in ~/.openmork/.env
...
# Configure
openmork config set terminal.backend singularity
openmork config set terminal.singularity_image ~/python.sif
```
...
modal setup
openmork config set terminal.backend modal
```
...
If a command needs sudo, you'll be prompted for your password (cached for the session). Or set `SUDO_PASSWORD` in `~/.openmork/.env`.

:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.openmork/.env`.
:::
