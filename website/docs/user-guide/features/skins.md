...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the OPENMORK CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the OPENMORK CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
/skin mytheme        # switch to a custom skin from ~/.openmork/skins/mytheme.yaml
```

Or set the default skin in `~/.openmork/config.yaml`:
...
Create YAML files under `~/.openmork/skins/`. User skins inherit missing values from the built-in `default` skin.
...
- Built-in skins load from `openmork_cli/skin_engine.py`.
