...
OpenMork is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, OPENMORK checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.
...
The `OPENMORK_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.openmork/config.yaml`:
...
:::tip
Use `openmork config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, OPENMORK controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.openmork/.env`:
...
For more flexible authorization, OPENMORK includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `openmork pairing approve <platform> <code>` on the CLI
...
openmork pairing list

# Approve a pairing code
openmork pairing approve telegram ABC12DEF

# Revoke a user's access
openmork pairing revoke telegram 123456789

# Clear all pending codes
openmork pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.openmork/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, OPENMORK applies strict security hardening to every container.
...
Container resources are configurable in `~/.openmork/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.openmork/sandboxes/docker/<task_id>/`
...
4. **Store secrets securely** — keep API keys in `~/.openmork/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.openmork/logs/` for unauthorized access attempts
10. **Keep updated** — run `openmork update` regularly for security patches

### Securing API Keys

```bash
# Set proper permissions on the .env file
chmod 600 ~/.openmork/.env
...
terminal:
  ...
  ssh_user: "openmork"
  ssh_key: "~/.ssh/openmork_agent_key"
```
