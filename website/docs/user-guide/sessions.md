...
openmork automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.openmork/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.openmork/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`openmork` or `openmork chat`) |
...
openmork -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
openmork -c "my project"   # → resumes "my project #3"
```
...
openmork --resume 20250305_091523_a1b2c3d4
openmork -r 20250305_091523_a1b2c3d4

# Resume by title
openmork --resume "refactoring auth"

# Or with the chat subcommand
openmork chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `openmork sessions list`.

### Conversation Recap on Resume

When you resume a session, OPENMORK displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a OPENMORK session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.openmork/config.yaml`:
...
```bash
openmork sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), OPENMORK creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`openmork -c "my project"`), it automatically picks the most recent session in the lineage.
...
OPENMORK provides a full set of session management commands via `openmork sessions`:
...
# Filter by platform
openmork sessions list --source telegram

# Show more sessions
openmork sessions list --limit 50
```
...
openmork sessions export backup.jsonl

# Export sessions from a specific platform
openmork sessions export telegram-history.jsonl --source telegram

# Export a single session
openmork sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
openmork sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
openmork sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
openmork sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
openmork sessions prune

# Custom age threshold
openmork sessions prune --older-than 30

# Only prune sessions from a specific platform
openmork sessions prune --source telegram --older-than 60

# Skip confirmation
openmork sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`openmork insights`](/docs/reference/cli-commands#openmork-insights).
...
|------|------|-------------|
| SQLite database | `~/.openmork/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.openmork/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.openmork/sessions/sessions.json` | Maps session keys to active session IDs |
...
openmork sessions delete <session_id>
