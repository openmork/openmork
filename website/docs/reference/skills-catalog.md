...
OPENMORK ships with a large built-in skill library copied into `~/.openmork/skills/` on install. This page catalogs the bundled skills that live in the repository under `skills/`.
...
| Skill | Description | Path |
|-------|-------------|------|
| `google-workspace` | Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration via Python. Uses OAuth2 with automatic token refresh. No external binaries needed — runs entirely with Google's Python client libraries in the OPENMORK venv. | `productivity/google-workspace` |
...
| `plan` | Plan mode for OPENMORK — inspect context, write a markdown plan into `.openmork/plans/` in the active workspace/backend working directory, and do not execute the work. | `software-development/plan` |
