...
is persisted in ~/.openmork/auth.json with cross-process file locking.
...
from openmork_cli.config import get_openmork_home, get_config_path
...
DEFAULT_NOUS_CLIENT_ID = "openmork-cli"
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `openmork model` to re-authenticate."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("OPENMORK_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.openmork/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    return get_openmork_home() / "auth.json"
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `openmork logout`.
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.openmork/auth.json (not ~/.codex/)
#
# OPENMORK maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from OPENMORK auth store (~/.openmork/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `openmork login` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `openmork login` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `openmork login` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `openmork login` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to OPENMORK auth store (~/.openmork/auth.json)."""
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to OPENMORK auth store automatically.
    """
    refresh_token = tokens.get("refresh_token")
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from OPENMORK's own Codex token store."""
    try:
        ...
        if cli_tokens:
            logger.info("Migrating Codex credentials from ~/.codex/ to OPENMORK auth store")
            print("⚠️  Migrating Codex credentials to OPENMORK's own auth store.")
            print("   This avoids conflicts with Codex CLI and VS Code.")
            print("   Run `openmork login` to create a fully independent session.\n")
    ...
    refresh_timeout_seconds = float(os.getenv("OPENMORK_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other OPENMORK processes
    ...
    base_url = (
        os.getenv("OPENMORK_CODEX_BASE_URL", "").strip().rstrip("/")
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("OPENMORK_CA_BUNDLE")
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip OPENMORK models — they're not reliable for agentic tool-calling
            if "openmork" in mid.lower():
                continue
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("OPENMORK is not logged into Nous Portal.",
                            provider="nous", relogin_required=True)

        portal_base_url = (
            _optional_base_url(state.get("portal_base_url"))
            or os.getenv("OPENMORK_PORTAL_BASE_URL")
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for `openmork status` output."""
...
def detect_external_credentials() -> List[Dict[str, Any]]:
    """Scan for credentials from other CLI tools that OPENMORK can reuse.

    Returns a list of dicts, each with:
      - provider: str   -- OPENMORK provider id (e.g. "openai-codex")
    ...
    if cli_tokens:
        ...
        found.append({
            "provider": "openai-codex",
            "path": str(codex_path),
            "label": f"Codex CLI credentials found ({codex_path}) — run `openmork login` to create a separate session",
        })
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from openmork_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'openmork model' or 'openmork setup' instead."""
    print("The 'openmork login' command has been removed.")
    print("Use 'openmork model' to select a provider and model,")
    print("or 'openmork setup' for full interactive setup.")
    raise SystemExit(0)


def _login_openai_codex(args, pconfig: ProviderConfig) -> None:
    """OpenAI Codex login via device code flow. Tokens stored in ~/.openmork/auth.json."""

    # Check for existing OPENMORK-owned credentials
    try:
        existing = resolve_codex_runtime_credentials()
        print("Existing Codex credentials found in OPENMORK auth store.")
        try:
    ...
    if cli_tokens:
        ...
        print("OPENMORK will create its own session to avoid conflicts with Codex CLI / VS Code.")
        try:
        ...
        if do_import in ("y", "yes"):
            _save_codex_tokens(cli_tokens)
            base_url = os.getenv("OPENMORK_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
            config_path = _update_config_for_provider("openai-codex", base_url)
            print()
            print("Credentials imported. Note: if Codex CLI refreshes its token,")
            print("OPENMORK will keep working independently with its own session.")
            print(f"  Config updated: {config_path} (model.provider=openai-codex)")
            return

    # Run a fresh device code flow — OPENMORK gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(OPENMORK creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to OPENMORK auth store
    _save_codex_tokens(creds["tokens"], creds.get("last_refresh"))
    config_path = _update_config_for_provider("openai-codex", creds.get("base_url", DEFAULT_CODEX_BASE_URL))
    print()
    print("Login successful!")
    print(f"  Auth state: ~/.openmork/auth.json")
    print(f"  Config updated: {config_path} (model.provider=openai-codex)")
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    print(f"Starting OPENMORK login via {pconfig.name}...")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target):
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            print("OPENMORK will use OpenRouter for inference.")
        else:
            print("Run `openmork model` or configure an API key to use OPENMORK.")
    else:
