#!/usr/bin/env python3
"""
OPENMORK CLI - Main entry point.

Usage:
    openmork                     # Interactive chat (default)
    openmork chat                # Interactive chat
    openmork gateway             # Run gateway in foreground
    openmork gateway start       # Start gateway as service
    openmork gateway stop        # Stop gateway service
    openmork gateway status      # Show gateway status
    openmork gateway install     # Install gateway service
    openmork gateway uninstall   # Uninstall gateway service
    openmork setup               # Interactive setup wizard
    openmork logout              # Clear stored authentication
    openmork status              # Show status of all components
    openmork cron                # Manage cron jobs
    openmork cron list           # List cron jobs
    openmork cron status         # Check if cron scheduler is running
    openmork doctor              # Check configuration and dependencies
    openmork honcho setup                    # Configure Honcho AI memory integration
    openmork honcho status                   # Show Honcho config and connection status
    ...
    openmork honcho tokens --context N       # Set session.context() token cap
    openmork honcho tokens --dialectic N     # Set dialectic result char cap
    openmork honcho identity                 # Show AI peer identity representation
    openmork honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    ...
    openmork claw migrate --dry-run  # Preview migration without changes
"""
...
load_openmork_dotenv(project_env=PROJECT_ROOT / '.env')

# Point mini-swe-agent at ~/.openmork/ so it shares our config
os.environ.setdefault("MSWEA_GLOBAL_CONFIG_DIR", str(get_openmork_home()))
...
from openmork_cli import __version__, __release_date__
...
def _has_any_provider_configured() -> bool:
    ...
    from openmork_cli.config import get_env_path, get_openmork_home
    from openmork_cli.auth import get_auth_status
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'openmork sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like OPENMORK isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  openmork setup")
        print()

        from openmork_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'openmork setup' at any time to configure.")
        sys.exit(1)
    ...
    if getattr(args, "yolo", False):
        os.environ["OPENMORK_YOLO_MODE"] = "1"
...
def cmd_gateway(args):
    ...
    from openmork_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from openmork_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with OPENMORK?")
        print()
    ...
    session_dir = get_openmork_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: openmork gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        print("⚠ Pairing may not have completed. Run 'openmork whatsapp' to try again.")
...
def cmd_model(args):
    ...
    from openmork_cli.config import load_config, save_config, get_env_value, save_env_value
    ...
    effective_provider = (
        config_provider
        or os.getenv("OPENMORK_INFERENCE_PROVIDER")
        or "auto"
    )
    try:
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from openmork_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from openmork_cli.config import get_env_value, save_env_value
    ...
    from openmork_cli.models import model_ids
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from openmork_cli.codex_models import get_codex_model_ids
    from openmork_cli.config import get_env_value, save_env_value
    import argparse
...
def _model_flow_custom(config):
    ...
    """
    from openmork_cli.auth import _save_model_choice, deactivate_provider
    from openmork_cli.config import get_env_value, save_env_value, load_config, save_config
    ...
    if model_name:
        ...
        if base_url or api_key:
            deactivate_provider()
        print("Endpoint saved. Use `/model` in chat or `openmork model` to set a model.")
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from openmork_cli.auth import _save_model_choice, deactivate_provider
    from openmork_cli.config import save_env_value, load_config, save_config
    from openmork_cli.models import fetch_api_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    from openmork_cli.config import (
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print("    OPENMORK will use Claude's credential store directly instead of copying a setup-token into ~/.openmork/.env.")
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from openmork_cli.models import _PROVIDER_MODELS
    ...
    if selected:
        ...
        # providers if the user switches without running 'openmork model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate OPENMORK CLI with a provider."""
    from openmork_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from openmork_cli.auth import logout_command
    logout_command(args)


def cmd_status(args):
    """Show status of all components."""
    from openmork_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from openmork_cli.cron import cron_command
    cron_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from openmork_cli.doctor import run_doctor
    run_doctor(args)


def cmd_config(args):
    """Configuration management."""
    from openmork_cli.config import config_command
    config_command(args)
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("openmork-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if OPENMORK behaves unexpectedly.")
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but OPENMORK couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but OPENMORK couldn't drop the saved stash entry.")
    ...
    print("  Review `git diff` / `git status` if OPENMORK behaves unexpectedly.")
    return True
    ...
    try:
        ...
        if needs_migration:
            ...
            if response in ('', 'y', 'yes'):
                ...
                print()
                print("Skipped. Run 'openmork config migrate' later to configure.")
        else:
        ...
        try:
            check = subprocess.run(
                ["systemctl", "--user", "is-active", "openmork-gateway"],
            ...
            if check.stdout.strip() == "active":
                ...
                restart = subprocess.run(
                    ["systemctl", "--user", "restart", "openmork-gateway"],
                ...
                if restart.returncode == 0:
                    ...
                    print("  Try manually: openmork gateway restart")
        except (FileNotFoundError, subprocess.TimeoutExpired):
            pass  # No systemd (macOS, WSL1, etc.) — skip silently
        
        print()
        print("Tip: You can now select a provider and model:")
        print("  openmork model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``openmork -c Pokemon Agent Dev`` without quoting the
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
    openmork                        Start interactive chat
    openmork chat -q "Hello"        Single query mode
    openmork -c                     Resume the most recent session
    openmork -c "my project"        Resume a session by name (latest in lineage)
    openmork --resume <session_id>  Resume a specific session by ID
    openmork setup                  Run setup wizard
    openmork logout                 Clear stored authentication
    openmork model                  Select default model
    openmork config                 View configuration
    ...
    openmork sessions browse        Interactive session picker
    openmork sessions rename ID T   Rename/title a session
    openmork update                 Update to latest version

For more help on a command:
    openmork <command> --help
"""
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for OPENMORK CLI"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: openmork-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    def cmd_pairing(args):
        from openmork_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            from openmork_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from openmork_cli.skills_hub import skills_command
            skills_command(args)
    ...
    honcho_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "each turn so OPENMORK knows who you are without you having to repeat yourself.\n\n"
    ...
    honcho_subparsers.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to OPENMORK Honcho",
    )
    ...
    def cmd_tools(args):
        from openmork_cli.tools_config import tools_command
        tools_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch openmork --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            openmork_bin = shutil.which("openmork")
            if openmork_bin:
                os.execvp(openmork_bin, ["openmork", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "openmork_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to OPENMORK"
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to OPENMORK",
    ...
    def cmd_claw(args):
        from openmork_cli.claw import claw_command
        claw_command(args)
    ...
    # e.g. ``openmork -c Pokemon Agent Dev`` → ``openmork -c 'Pokemon Agent Dev'``
