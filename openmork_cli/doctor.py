...
from openmork_cli.config import get_project_root, get_openmork_home, get_env_path

PROJECT_ROOT = get_project_root()
OPENMORK_HOME = get_openmork_home()

# Load environment variables from ~/.openmork/.env so API key checks work
...
os.environ.setdefault("MSWEA_GLOBAL_CONFIG_DIR", str(OPENMORK_HOME))
os.environ.setdefault("MSWEA_SILENT_STARTUP", "1")

from openmork_cli.colors import Colors, color
from openmork_constants import OPENROUTER_MODELS_URL
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.openmork/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `openmork`.
    os.environ.setdefault("OPENMORK_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 OPENMORK Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.openmork/.env (primary location for user config)
    env_path = OPENMORK_HOME / '.env'
    if env_path.exists():
        check_ok("~/.openmork/.env file exists")
        ...
        if _has_provider_env_config(content):
            ...
            check_warn("No API key found in ~/.openmork/.env")
            issues.append("Run 'openmork setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            check_fail("~/.openmork/.env file missing")
            if should_fix:
                env_path.parent.mkdir(parents=True, exist_ok=True)
                env_path.touch()
                check_ok("Created empty ~/.openmork/.env")
                check_info("Run 'openmork setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'openmork setup' to create one")
                issues.append("Run 'openmork setup' to create .env")
    
    # Check ~/.openmork/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = OPENMORK_HOME / 'config.yaml'
    if config_path.exists():
        check_ok("~/.openmork/config.yaml exists")
    else:
        ...
        if fallback_config.exists():
            ...
            if should_fix and example_config.exists():
                ...
                check_ok("Created ~/.openmork/config.yaml from cli-config.yaml.example")
                fixed_count += 1
            elif should_fix:
                check_warn("config.yaml not found and no example to copy from")
                manual_issues.append("Create ~/.openmork/config.yaml manually")
            else:
    ...
    try:
        from openmork_cli.auth import get_nous_auth_status, get_codex_auth_status
    ...
    openmork_home = OPENMORK_HOME
    if openmork_home.exists():
        check_ok("~/.openmork directory exists")
    else:
        if should_fix:
            openmork_home.mkdir(parents=True, exist_ok=True)
            check_ok("Created ~/.openmork directory")
            fixed_count += 1
        else:
            check_warn("~/.openmork not found", "(will be created on first use)")
    
    # Check expected subdirectories
    expected_subdirs = ["cron", "sessions", "logs", "skills", "memories"]
    for subdir_name in expected_subdirs:
        subdir_path = openmork_home / subdir_name
        if subdir_path.exists():
            check_ok(f"~/.openmork/{subdir_name}/ exists")
        else:
            if should_fix:
                subdir_path.mkdir(parents=True, exist_ok=True)
                check_ok(f"Created ~/.openmork/{subdir_name}/")
                fixed_count += 1
            else:
                check_warn(f"~/.openmork/{subdir_name}/ not found", "(will be created on first use)")
    
    # Check for SOUL.md persona file
    soul_path = openmork_home / "SOUL.md"
    if soul_path.exists():
        content = soul_path.read_text(encoding="utf-8").strip()
        # Check if it's just the template comments (no real content)
        lines = [l for l in content.splitlines() if l.strip() and not l.strip().startswith(("<!--", "-->", "#"))]
        if lines:
            check_ok("~/.openmork/SOUL.md exists (persona configured)")
        else:
            check_info("~/.openmork/SOUL.md exists but is empty — edit it to customize personality")
    else:
        check_warn("~/.openmork/SOUL.md not found", "(create it to give OPENMORK a custom personality)")
        if should_fix:
            ...
            check_ok("Created ~/.openmork/SOUL.md with basic template")
            fixed_count += 1
    
    # Check memory directory
    memories_dir = openmork_home / "memories"
    if memories_dir.exists():
        check_ok("~/.openmork/memories/ directory exists")
        ...
        check_warn("~/.openmork/memories/ not found", "(will be created on first use)")
        if should_fix:
            memories_dir.mkdir(parents=True, exist_ok=True)
            check_ok("Created ~/.openmork/memories/")
            fixed_count += 1
    
    # Check SQLite session store
    state_db_path = openmork_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            conn.close()
            check_ok(f"~/.openmork/state.db exists ({count} sessions)")
        except Exception as e:
            check_warn(f"~/.openmork/state.db exists but has issues: {e}")
    else:
        check_info("~/.openmork/state.db not created yet (will be created on first session)")
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'openmork setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    hub_dir = OPENMORK_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: openmork skills list)")
    ...
    if github_token:
        ...
        check_warn("No GITHUB_TOKEN", "(60 req/hr rate limit — set in ~/.openmork/.env for better rates)")
    ...
    try:
        ...
        if not GLOBAL_CONFIG_PATH.exists():
            check_warn("Honcho config not found", f"run: openmork honcho setup")
        elif not hcfg.enabled:
            check_info("Honcho disabled (set enabled: true in ~/.honcho/config.json to activate)")
        elif not hcfg.api_key:
            check_fail("Honcho API key not set", "run: openmork honcho setup")
            issues.append("No Honcho API key — run 'openmork honcho setup'")
        else:
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'openmork doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
