...
Pure display functions with no OPENMORKCLI state dependency.
"""
...
from openmork_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
def get_available_skills() -> Dict[str, List[str]]:
    """Scan ~/.openmork/skills/ and return skills grouped by category."""
    import os
...
def check_for_updates() -> Optional[int]:
    ...
    ``~/.openmork/.update_check``).  Returns the number of commits behind,
...
def build_welcome_banner(console: Console, model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else OPENMORK_CADUCEUS
    except Exception:
        _bskin = None
        _hero = OPENMORK_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else OPENMORK_AGENT_LOGO
        console.print(_logo)
        console.print()
