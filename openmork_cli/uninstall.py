...
- Keep data: Remove code but keep ~/.openmork/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove OPENMORK PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                ...
                # Remove any PATH line containing openmork
                if 'openmork' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the openmork wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "openmork",
        Path("/usr/local/bin/openmork"),
    ]
...
def uninstall_gateway_service():
    ...
    service_file = Path.home() / ".config" / "systemd" / "user" / "openmork-gateway.service"
    ...
    try:
        ...
        # Disable the service
        subprocess.run(
            ["systemctl", "--user", "disable", "openmork-gateway"],
            capture_output=True,
            check=False
        )
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.openmork/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.openmork/ for future reinstall
    """
    ...
    print(f"  Config:  {openmork_home / 'config.yaml'}")
    print(f"  Secrets: {openmork_home / '.env'}")
    print(f"  Data:    {openmork_home / 'cron/'}, {openmork_home / 'sessions/'}, {openmork_home / 'logs/'}")
    print()
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL OPENMORK data!", Colors.RED, Colors.BOLD))
        print(color("   Including: configs, API keys, sessions, scheduled jobs, logs", Colors.RED))
    else:
        print("This will remove the OPENMORK code but keep your configuration and data.")
    ...
    log_info("Removing openmork command...")
    ...
    # 5. Optionally remove ~/.openmork/ data directory
    if full_uninstall:
        log_info("Removing configuration and data...")
        try:
            if openmork_home.exists():
                shutil.rmtree(openmork_home)
                log_success(f"Removed {openmork_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {openmork_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {openmork_home}")
