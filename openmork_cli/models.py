...
Add, remove, or reorder entries here — both `openmork setup` and
`openmork` provider-selection will pick up the change automatically.
"""
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        nous:openmork-3                           →  ("nous", "openmork-3")
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to OPENMORK' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``openmork_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def provider_model_ids(provider: Optional[str]) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from openmork_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
