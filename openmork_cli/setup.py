...
def _setup_provider_model_selection(config, provider_id, current_model, prompt_choice, prompt_fn):
    ...
    from openmork_cli.config import get_env_value
...
from openmork_cli.colors import Colors, color
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ OPENMORK Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure OPENMORK using environment variables or config commands:")
    print_info("  openmork config set model.provider custom")
    print_info("  openmork config set model.base_url http://localhost:8080/v1")
    print_info("  openmork config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'openmork setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning(f"  Skipped (configure later with 'openmork setup')")


def _print_setup_summary(config: dict, openmork_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'openmork setup' to configure"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'openmork setup tools' to configure them,"
        )
        print_warning("or edit ~/.openmork/.env directly to add the missing API keys.")
        print()
    ...
    print(color("📁 All your files are in ~/.openmork/:", Colors.CYAN, Colors.BOLD))
    print()
    print(f"   {color('Settings:', Colors.YELLOW)}  {get_config_path()}")
    print(f"   {color('API Keys:', Colors.YELLOW)}  {get_env_path()}")
    print(
        f"   {color('Data:', Colors.YELLOW)}      {openmork_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('openmork setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('openmork setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('openmork setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('openmork setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('openmork setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('openmork config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('openmork config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('openmork config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('openmork', Colors.GREEN)}              Start chatting")
    print(f"   {color('openmork gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('openmork doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `openmork tools` and `openmork setup tools`).
...
def setup_model_provider(config: dict):
    ...
    if not has_any_provider:
        print_warning("An inference provider is required for OPENMORK to work.")
        print()
    ...
    if provider_idx == 0:  # Nous Portal (OAuth)
        ...
        try:
            from openmork_cli.auth import _login_nous, ProviderConfig
            import argparse
            ...
            print_info("You can try again later with: openmork model")
        ...
        try:
            from openmork_cli.auth import deactivate_provider
        ...
        config_path = (
            Path(os.environ.get("OPENMORK_HOME", Path.home() / ".openmork")) / "config.yaml"
        )
        try:
            disk_cfg = {}
        ...
        if base_url:
            ...
            config_path = (
                Path(os.environ.get("OPENMORK_HOME", Path.home() / ".openmork"))
                / "config.yaml"
            )
            try:
                disk_cfg = {}
        ...
        if api_key:
            ...
            from openmork_cli.auth import detect_zai_endpoint
        ...
        from openmork_cli.auth import PROVIDER_REGISTRY
        from openmork_cli.config import save_anthropic_api_key, save_anthropic_oauth_token
    ...
    if selected_provider in _vision_backends:
        # If the user just selected a backend OPENMORK can already use for
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one OPENMORK can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'openmork setup' or configure AUXILIARY_VISION_* settings")
    ...
    if selected_provider != "custom":  # Custom already prompted for model name
        ...
        if selected_provider == "nous" and nous_models:
            ...
            from openmork_cli.models import provider_model_ids
            ...
            from openmork_cli.models import model_ids, menu_labels
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where OPENMORK runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using OPENMORK via Telegram/Discord, this is where")
        print_info(
...
def setup_agent_settings(config: dict):
    ...
    current_max = get_env_value("OPENMORK_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("OPENMORK_MAX_ITERATIONS", str(max_iter))
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with OPENMORK from anywhere.")
    print()
    ...
    if not existing_telegram and prompt_yes_no("Set up Telegram bot?", False):
        ...
        if token:
            ...
            print()
            print_info("📬 Home Channel: where OPENMORK delivers cron job results,")
    ...
    if not existing_slack and prompt_yes_no("Set up Slack bot?", False):
        ...
        print()
        print_info(
            "   Full guide: https://openmork.local/docs/user-guide/messaging/slack/"
        )
        print()
    ...
    if not existing_whatsapp and prompt_yes_no("Set up WhatsApp?", False):
        ...
        print_info("Requires Node.js. Run 'openmork whatsapp' for guided setup.")
        print()
        if prompt_yes_no("Enable WhatsApp now?", True):
            save_env_value("WHATSAPP_ENABLED", "true")
            print_success("WhatsApp enabled")
            print_info("Run 'openmork whatsapp' to choose your mode (separate bot number")
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     openmork config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        from openmork_cli.gateway import (
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: openmork gateway install")
            else:
                print_info("  You can install later: openmork gateway install")
                if _is_linux:
                    print_info("  Or as a boot-time service: sudo openmork gateway install --system")
                print_info("  Or run in foreground:  openmork gateway")
        else:
            print_info("Start the gateway to bring your bots online:")
            print_info("   openmork gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `openmork setup tools` and `openmork tools` use the same flow:
...
def _offer_openclaw_migration(openmork_home: Path) -> bool:
    ...
    print_info("OPENMORK can import your settings, memories, skills, and API keys.")
    print()
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "openclaw_to_openmork", _OPENCLAW_SCRIPT
        )
        ...
        migrator = mod.Migrator(
            ...
            target_root=openmork_home.resolve(),
            execute=True,
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in OPENMORK.")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      openmork setup           — full or quick (auto-detected)
      openmork setup model     — just model/provider
      openmork setup terminal  — just terminal backend
      openmork setup gateway   — just messaging platforms
      openmork setup tools     — just tool configuration
      openmork setup agent     — just agent settings
    """
    ensure_openmork_home()

    config = load_config()
    openmork_home = get_openmork_home()
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ OPENMORK Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    from openmork_cli.auth import get_active_provider
    ...
    if is_existing:
        ...
        print_success("You already have OPENMORK configured.")
        print()
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, openmork_home)
            return
        elif choice == 1:
            ...
            print_info("Exiting. Run 'openmork setup' again when ready.")
            return
        elif choice == 9:
            print_info("Exiting. Run 'openmork setup' again when ready.")
            return
            ...
            func(config)
            save_config(config)
            _print_setup_summary(config, openmork_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        if _offer_openclaw_migration(openmork_home):
    ...
    print_info(f"Data folder:  {openmork_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'openmork config edit'")
...
def _run_quick_setup(config: dict, openmork_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'openmork setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect OPENMORK to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'openmork setup gateway'.")
