"""openmork claw — OpenClaw migration commands.

Usage:
    openmork claw migrate              # Interactive migration from ~/.openclaw
    openmork claw migrate --dry-run    # Preview what would be migrated
    openmork claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
"""
...
from openmork_cli.config import get_openmork_home, get_config_path, load_config, save_config
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_openmork.py script in known locations."""
...
def claw_command(args):
    """Route openmork claw subcommands."""
    action = getattr(args, "claw_action", None)

    if action == "migrate":
        _cmd_migrate(args)
    else:
        print("Usage: openmork claw migrate [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to OPENMORK")
        print()
        print("Run 'openmork claw migrate --help' for migration options.")


def _cmd_migrate(args):
    """Run the OpenClaw → OPENMORK migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info(f"You can specify a custom path: openmork claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {openmork_home}")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  openmork claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
