...
- openmork config          - Show current configuration
- openmork config edit     - Open config in editor
- openmork config set      - Set a specific value
- openmork config wizard   - Re-run setup wizard
"""
...
from openmork_cli.default_soul import DEFAULT_SOUL_MD
...
def get_openmork_home() -> Path:
    """Get the OPENMORK home directory (~/.openmork)."""
    return Path(os.getenv("OPENMORK_HOME", Path.home() / ".openmork"))

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_openmork_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_openmork_home() / ".env"
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into OPENMORK_HOME if the user doesn't have one yet."""
...
def ensure_openmork_home():
    """Ensure ~/.openmork directory structure exists with secure permissions."""
...
DEFAULT_CONFIG = {
    ...
    # This section is only needed for openmork-specific overrides; everything else
...
OPTIONAL_ENV_VARS = {
    ...
    # OPENMORK_TOOL_PROGRESS and OPENMORK_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "OPENMORK_TOOL_PROGRESS": {
    ...
    "OPENMORK_PREFILL_MESSAGES_FILE": {
    ...
    "OPENMORK_EPHEMERAL_SYSTEM_PROMPT": {
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("OPENMORK_TOOL_PROGRESS")
            old_mode = get_env_value("OPENMORK_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from OPENMORK_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from OPENMORK_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("OPENMORK_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from OPENMORK_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: openmork config set <key> <value>")
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.openmork/config.yaml."""
    import copy
...
#   openai-codex (OAuth — openmork login) — OpenAI Codex
#   nous         (OAuth — openmork login) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.openmork/config.yaml."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.openmork/.env."""
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.openmork/.env."""
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.openmork/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ OPENMORK Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  openmork config edit     # Edit config file", Colors.DIM))
    print(color("  openmork config set <key> <value>", Colors.DIM))
    print(color("  openmork setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or not value:
            print("Usage: openmork config set <key> <value>")
            print()
            print("Examples:")
            print("  openmork config set model anthropic/claude-sonnet-4")
            print("  openmork config set terminal.backend docker")
            print("  openmork config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'openmork config migrate' to add them")
        ...
        print("  openmork config           Show current configuration")
        print("  openmork config edit      Open config in editor")
        print("  openmork config set <key> <value>   Set a config value")
        print("  openmork config check     Check for missing/outdated config")
        print("  openmork config migrate   Update config with new options")
        print("  openmork config path      Show config file path")
        print("  openmork config env-path  Show .env file path")
        sys.exit(1)
