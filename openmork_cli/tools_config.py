...
Saves per-platform tool configuration to ~/.openmork/config.yaml under
...
# They're still in _OPENMORK_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
_DEFAULT_OFF_TOOLSETS = {"moa", "homeassistant", "rl"}

# Platform display config
PLATFORMS = {
    "cli":      {"label": "🖥️  CLI",       "default_toolset": "openmork-cli"},
    "telegram": {"label": "📱 Telegram",   "default_toolset": "openmork-telegram"},
    "discord":  {"label": "💬 Discord",    "default_toolset": "openmork-discord"},
    "slack":    {"label": "💼 Slack",      "default_toolset": "openmork-slack"},
    "whatsapp": {"label": "📱 WhatsApp",   "default_toolset": "openmork-whatsapp"},
    "signal":   {"label": "📡 Signal",     "default_toolset": "openmork-signal"},
    "email":    {"label": "📧 Email",      "default_toolset": "openmork-email"},
}
...
def _prompt_toolset_checklist(platform_label: str, enabled: Set[str]) -> Set[str]:
    ...
    from openmork_cli.curses_ui import curses_checklist
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `openmork tools` and `openmork setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ OPENMORK Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    print()
    print(color("  Tool configuration saved to ~/.openmork/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'openmork' or gateway restart.", Colors.DIM))
    print()
