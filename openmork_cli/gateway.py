"""
Gateway subcommand for openmork CLI.

Handles: openmork gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
from openmork_cli.config import get_env_value, get_openmork_home, save_env_value
from openmork_cli.setup import (
...
SERVICE_NAME = "openmork-gateway"
SERVICE_DESCRIPTION = "OpenMork Gateway - Messaging Platform Integration"
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    openmork gateway uninstall")
    print_info("    sudo openmork gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so OPENMORK can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo openmork gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo openmork gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo openmork gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    return Path.home() / "Library" / "LaunchAgents" / "ai.openmork.gateway.plist"
...
def get_openmork_cli_path() -> str:
    """Get the path to the openmork CLI."""
    ...
    return f"{get_python_path()} -m openmork_cli.main"
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    openmork_cli = shutil.which("openmork") or f"{python_path} -m openmork_cli.main"
...
User={username}
Group={group_name}
ExecStart={python_path} -m openmork_cli.main gateway run --replace
...
Type=simple
ExecStart={python_path} -m openmork_cli.main gateway run --replace
ExecStop={openmork_cli} gateway stop
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Restart=on-failure
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current OPENMORK install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}openmork gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}openmork gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}openmork gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}openmork gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}openmork gateway start{scope_flag}")
...
def generate_launchd_plist() -> str:
    python_path = get_python_path()
    working_dir = str(PROJECT_ROOT)
    log_dir = get_openmork_home() / "logs"
...
<dict>
    <key>Label</key>
    <string>ai.openmork.gateway</string>
    
    <key>ProgramArguments</key>
    <array>
        <string>{python_path}</string>
        <string>-m</string>
        <string>openmork_cli.main</string>
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  openmork gateway status             # Check status")
    print("  tail -f ~/.openmork/logs/gateway.log  # View logs")
...
def launchd_start():
    subprocess.run(["launchctl", "start", "ai.openmork.gateway"], check=True)
    print("✓ Service started")

def launchd_stop():
    subprocess.run(["launchctl", "stop", "ai.openmork.gateway"], check=True)
...
def launchd_status(deep: bool = False):
    result = subprocess.run(
        ["launchctl", "list", "ai.openmork.gateway"],
        capture_output=True,
        text=True
    )
    ...
    if deep:
        log_file = get_openmork_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def run_gateway(verbose: bool = False, replace: bool = False):
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ OPENMORK Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your OpenMork",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address OPENMORK will use (e.g., openmork@gmail.com)."},
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_openmork_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'openmork pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: openmork pairing approve {platform} {code}")
                else:
                    print_info("  Skipped — configure later with 'openmork gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from openmork_cli.main import cmd_whatsapp
    import argparse
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"OPENMORKAgent\"")
    ...
    if any_configured:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if is_linux():
                        ...
                        print_info("Start manually: openmork gateway")
                except subprocess.CalledProcessError as e:
            ...
            if is_linux() or is_macos():
                ...
                print_info("  Run in foreground: openmork gateway")
    else:
        print()
        print_info("No platforms configured. Run 'openmork gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_linux():
            ...
            print("Run manually: openmork gateway run")
            sys.exit(1)
        ...
        if is_linux() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                print("  openmork gateway install")
                print("  sudo openmork gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  openmork gateway          # Run in foreground")
                print("  openmork gateway install  # Install as user service")
                print("  sudo openmork gateway install --system  # Install as boot-time system service")
