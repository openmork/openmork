...
from openmork_cli.auth import AuthError, resolve_provider
from openmork_cli.colors import Colors, color
from openmork_cli.config import get_env_path, get_env_value, get_openmork_home, load_config
from openmork_cli.models import provider_label
from openmork_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    ...
    print(
        ...
        f"{'logged in' if nous_logged_in else 'not logged in (run: openmork model)'}"
    )
    if nous_logged_in:
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: openmork model)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: openmork model)"
    ...
    if not terminal_env:
        ...
        # (openmork status doesn't go through cli.py's config loading)
        try:
            _cfg = load_config()
    ...
    jobs_file = get_openmork_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_openmork_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'openmork doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'openmork setup' to configure", Colors.DIM))
    print()
