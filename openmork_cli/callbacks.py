...
into prompt_toolkit's event loop. Each function takes the OPENMORKCLI instance
...
from openmork_cli.banner import cprint, _DIM, _RST
from openmork_cli.config import save_env_value_secure
...
def prompt_for_secret(cli, var_name: str, prompt: str, metadata=None) -> dict:
    ...
    The secret is stored in ~/.openmork/.env and never exposed to the model.
    """
    if not getattr(cli, "_app", None):
        ...
        cprint(f"\n{_DIM}  ✓ Stored secret in ~/.openmork/.env as {var_name}{_RST}")
        return {
            **stored,
            "skipped": False,
