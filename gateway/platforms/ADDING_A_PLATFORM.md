...
Checklist for integrating a new messaging platform into the OPENMORK gateway.
...
```python
"openmork-your-platform": {
    "description": "Your Platform bot toolset",
    "tools": _OPENMORK_CORE_TOOLS,
    "includes": []
},
```

And add it to the `openmork-gateway` composite:

```python
"openmork-gateway": {
    "includes": [..., "openmork-your-platform"]
}
```
...
## 12. Status Display (`openmork_cli/status.py`)
...
## 13. Gateway Setup Wizard (`openmork_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ openmork_cli/ toolsets.py \
  --include="*.py" -l | sort -u
