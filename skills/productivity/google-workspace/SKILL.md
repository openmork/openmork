...
description: Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration via Python. Uses OAuth2 with automatic token refresh. No external binaries needed — runs entirely with Google's Python client libraries in the OPENMORK venv.
version: 1.0.0
...
```bash
GSETUP="python ~/.openmork/skills/productivity/google-workspace/scripts/setup.py"
```
...
- Token is stored at `~/.openmork/google_token.json` and auto-refreshes.
- Pending OAuth session state/verifier are stored temporarily at `~/.openmork/google_oauth_pending.json` until exchange completes.
...
```bash
GAPI="python ~/.openmork/skills/productivity/google-workspace/scripts/google_api.py"
```
