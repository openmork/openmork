...
| Feature | `delegate_task` | Spawning `openmork` process |
|---------|-----------------|--------------------------|
...
- `openmork` CLI installed and on PATH
- API key configured in `~/.openmork/.env`
...
terminal(command="openmork --continue", background=true, pty=true)

# Resume a specific session by ID (shown on exit)
terminal(command="openmork --resume 20260225_143052_a1b2c3", background=true, pty=true)
```
...
```
terminal(command="openmork chat -q 'Research the latest GRPO training papers and write a summary to ~/research/grpo.md'", timeout=300)
```

Background for long tasks:
```
terminal(command="openmork chat -q 'Set up CI/CD for ~/myapp'", background=true)
# Returns session_id, monitor with process tool
```

## Mode 2: Interactive PTY Session

Launch a full interactive OPENMORK session with PTY for back-and-forth collaboration. You can send messages, review its work, give feedback, and steer it.

Note: OPENMORK uses prompt_toolkit for its CLI UI. Through a PTY, this works because ptyprocess provides a real terminal — input sent via `submit` arrives as keystrokes. The output log will contain ANSI escape sequences from the UI rendering — focus on the text content, not the formatting.

```
# Start interactive openmork in background with PTY
terminal(command="openmork", workdir="~/project", background=true, pty=true)
# Returns session_id
...
**Code review loop** — spawn openmork, send code for review, iterate on feedback:
```
terminal(command="openmork", workdir="~/project", background=true, pty=true)
...
```
terminal(command="openmork", background=true, pty=true)
...
```
# Agent A: backend
terminal(command="openmork", workdir="~/project/backend", background=true, pty=true)
process(action="submit", session_id="<agent-a>", data="Build a REST API for user management with CRUD endpoints")

# Agent B: frontend
terminal(command="openmork", workdir="~/project/frontend", background=true, pty=true)
...
```
terminal(command="openmork chat -q 'Research competitor landing pages and write a report to ~/research/competitors.md'", background=true)
terminal(command="openmork chat -q 'Audit security of ~/myapp and write findings to ~/myapp/SECURITY_AUDIT.md'", background=true)
process(action="list")
```
...
```
terminal(command="openmork chat -q 'Summarize this codebase' --model google/gemini-2.5-pro", workdir="~/project", background=true)
```
...
```
# Start openmork inside tmux
tmux new-session -d -s openmork-session -x 120 -y 40 "openmork"
sleep 10  # Wait for banner/startup

# Send messages
tmux send-keys -t openmork-session "your message here" Enter

# Read output
sleep 15  # Wait for LLM response
tmux capture-pane -t openmork-session -p

# Multi-turn: just send more messages and capture again
tmux send-keys -t openmork-session "follow-up message" Enter

# Exit when done
tmux send-keys -t openmork-session "/exit" Enter
tmux kill-session -t openmork-session
```
