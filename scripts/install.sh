...
check_node() {
    ...
    if [ -x "$OPENMORK_HOME/node/bin/node" ]; then
        export PATH="$OPENMORK_HOME/node/bin:$PATH"
        local found_ver=$("$OPENMORK_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (OPENMORK-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.openmork/node/..."
    ...
    # Place into ~/.openmork/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$OPENMORK_HOME/node"
    mkdir -p "$OPENMORK_HOME"
    mv "$extracted_dir" "$OPENMORK_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$OPENMORK_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$OPENMORK_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$OPENMORK_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$OPENMORK_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$OPENMORK_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.openmork/node/"
    HAS_NODE=true
}
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="openmork-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if OPENMORK behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up openmork command..."

    if [ "$USE_VENV" = true ]; then
        OPENMORK_BIN="$INSTALL_DIR/venv/bin/openmork"
    else
        OPENMORK_BIN="$(which openmork 2>/dev/null || echo "")"
        if [ -z "$OPENMORK_BIN" ]; then
            log_warn "openmork not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$OPENMORK_BIN" ]; then
        log_warn "openmork entry point not found at $OPENMORK_BIN"
    ...
    mkdir -p "$HOME/.local/bin"
    ln -sf "$OPENMORK_BIN" "$HOME/.local/bin/openmork"
    log_success "Symlinked openmork → ~/.local/bin/openmork"
    ...
    # Export for current session so openmork works immediately
    export PATH="$HOME/.local/bin:$PATH"

    log_success "openmork command ready"
}

copy_config_templates() {
    log_info "Setting up configuration files..."

    # Create ~/.openmork directory structure (config at top level, code in subdir)
    mkdir -p "$OPENMORK_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills,whatsapp/session}

    # Create .env at ~/.openmork/.env (top level, easy to find)
    if [ ! -f "$OPENMORK_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$OPENMORK_HOME/.env"
            log_success "Created ~/.openmork/.env from template"
        else
            touch "$OPENMORK_HOME/.env"
            log_success "Created ~/.openmork/.env"
        fi
    else
        log_info "~/.openmork/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.openmork/config.yaml (top level, easy to find)
    if [ ! -f "$OPENMORK_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$OPENMORK_HOME/config.yaml"
            log_success "Created ~/.openmork/config.yaml from template"
        fi
    else
        log_info "~/.openmork/config.yaml already exists, keeping it"
    fi
...
-->
SOUL_EOF
        log_success "Created ~/.openmork/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.openmork/"

    # Seed bundled skills into ~/.openmork/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.openmork/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.openmork/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$OPENMORK_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$OPENMORK_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.openmork/skills/"
        fi
    fi
}
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run 'openmork setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m openmork_cli.main setup < /dev/tty
    else
        python -m openmork_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$OPENMORK_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for OPENMORK to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$OPENMORK_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'openmork whatsapp' to pair via QR code..."
            echo ""
            read -p "Pair WhatsApp now? [Y/n] " -n 1 -r
            echo
            if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
                OPENMORK_CMD="$HOME/.local/bin/openmork"
                [ ! -x "$OPENMORK_CMD" ] && OPENMORK_CMD="openmork"
                $OPENMORK_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'openmork whatsapp' to pair."
        fi
    fi

    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'openmork gateway install' later."
        return 0
    fi
    ...
    if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
        OPENMORK_CMD="$HOME/.local/bin/openmork"
        if [ ! -x "$OPENMORK_CMD" ]; then
            OPENMORK_CMD="openmork"
        fi

        if command -v systemctl &> /dev/null; then
            log_info "Installing systemd service..."
            if $OPENMORK_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $OPENMORK_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: openmork gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: openmork gateway"
            fi
        else
            log_info "systemd not available — starting gateway in background..."
            nohup $OPENMORK_CMD gateway > "$OPENMORK_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.openmork/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: openmork gateway"
        fi
    else
        log_info "Skipped. Start the gateway later with: openmork gateway"
    fi
}
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.openmork/):${NC}"
    echo ""
    ...
    echo ""
    echo -e "   ${GREEN}openmork${NC}              Start chatting"
    echo -e "   ${GREEN}openmork setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}openmork config${NC}       View/edit configuration"
    echo -e "   ${GREEN}openmork config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}openmork gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}openmork update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    echo -e "${YELLOW}⚡ Reload your shell to use 'openmork' command:${NC}"
    echo ""
