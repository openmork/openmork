...
Configuration is loaded from ~/.openmork/config.yaml under the 'tts:' key.
...
DEFAULT_OUTPUT_DIR = str(Path(os.getenv("OPENMORK_HOME", Path.home() / ".openmork")) / "audio_cache")
MAX_TEXT_LENGTH = 4000


# ===========================================================================
# Config loader -- reads tts: section from ~/.openmork/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.openmork/config.yaml.
    ...
    try:
        ...
        logger.debug("openmork_cli.config not available, using default TTS config")
        return {}
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.openmork/config.yaml (tts: section).
    ...
    platform = os.getenv("OPENMORK_SESSION_PLATFORM", "").lower()
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": "Optional custom file path to save the audio. Defaults to ~/.openmork/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
