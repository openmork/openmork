...
# 5. Symlinks the 'openmork' CLI command into ~/.local/bin
...
# ============================================================================
# PATH setup — symlink openmork into ~/.local/bin
# ============================================================================

echo -e "${CYAN}→${NC} Setting up openmork command..."

OPENMORK_BIN="$SCRIPT_DIR/venv/bin/openmork"
mkdir -p "$HOME/.local/bin"
ln -sf "$OPENMORK_BIN" "$HOME/.local/bin/openmork"
echo -e "${GREEN}✓${NC} Symlinked openmork → ~/.local/bin/openmork"
...
# ============================================================================
# Seed bundled skills into ~/.openmork/skills/
# ============================================================================

OPENMORK_SKILLS_DIR="${OPENMORK_HOME:-$HOME/.openmork}/skills"
mkdir -p "$OPENMORK_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.openmork/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$OPENMORK_SKILLS_DIR/" 2>/dev/null || true
...
echo ""
echo "Other commands:"
echo "  openmork status        # Check configuration"
echo "  openmork gateway install # Install gateway service (messaging + cron)"
echo "  openmork cron list     # View scheduled jobs"
echo "  openmork doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m openmork_cli.main setup
fi
