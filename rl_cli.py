...
if tinker_atropos_dir.exists():
    ...
    os.environ['OPENMORK_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to openmork directory if submodule not found
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.openmork/config.yaml if not provided)
    ...
    """
    # Load config from ~/.openmork/config.yaml
    config = load_openmork_config()
    ...
    if check_server:
        ...
        if ok:
            ...
            if missing:
                ...
                print("   Add them to ~/.openmork/.env")
            else:
