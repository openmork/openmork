...
2. **Cross-platform compatibility** — Windows, macOS, different Linux distros, different terminal emulators. We want OPENMORK to work everywhere.
...
Bundled skills (in `skills/`) ship with every OPENMORK install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `openmork skills browse` (labeled "official") and install it with `openmork skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `openmork skills install`.
...
```bash
mkdir -p ~/.openmork/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.openmork/config.yaml
touch ~/.openmork/.env

# Add at minimum an LLM provider key:
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.openmork/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/openmork" ~/.local/bin/openmork
...
├── toolsets.py               # Tool groupings and presets (openmork-cli, openmork-telegram, etc.)
├── openmork_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── openmork_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.openmork/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (openmork.local)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.openmork/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.openmork/`)

| Path | Purpose |
|------|---------|
| `~/.openmork/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.openmork/.env` | API keys and secrets |
| `~/.openmork/auth.json` | OAuth credentials (Nous Portal) |
| `~/.openmork/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.openmork/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.openmork/state.db` | SQLite session database |
| `~/.openmork/sessions/` | JSON session logs |
| `~/.openmork/cron/` | Scheduled job data |
| `~/.openmork/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`openmork_state.py`) with full-text search and unique session titles. JSON logs go to `~/.openmork/sessions/`.
...
Four fields are supported under `metadata.openmork`:
...
The user may skip setup and keep loading the skill. OPENMORK only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `openmork setup` or update `~/.openmork/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing OPENMORK tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `openmork --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

OPENMORK uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.openmork/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `openmork_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `openmork_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

OPENMORK runs on Linux, macOS, and Windows. When writing code that touches the OS:
...
OPENMORK has terminal access. Security matters.
...
2. **Test manually**: Run `openmork` and exercise the code path you changed
